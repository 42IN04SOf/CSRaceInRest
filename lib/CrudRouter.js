...
module.exports = function(router, modelName, repository, options, html)
{	
	...
	var viewHandler = function(viewObject) {
		return function(req, res) {
			var publicData = [];
			if(Array.isArray(req[modelName])) {
				for(element of req[modelName]) {
	...
	if(options.readById) {
		// GET /entity/:id		
		router.get('/:' + modelName + 'Id', viewHandler(html.detail));
	}
